...
class ResNet(nn.Module):
    ...
    def __init__(self, depth, num_filters, block_name='BasicBlock', num_classes=10):
        ...
        # 根据 block_name 动态选择 BasicBlock 或 Bottleneck
        if block_name.lower() == 'basicblock':
            # 只有满足 depth 为 6n + 2 的形式才是合法的
            assert (depth - 2) % 6 == 0, 'When use basicblock, depth should be 6n+2, e.g. 20, 32, 44, 56, 110, 1202'
            n = (depth - 2) // 6
            block = BasicBlock
        elif block_name.lower() == 'bottleneck':
            # 只有满足 depth 为 9n + 2 的形式才是合法的
...
def resnet18(**kwargs):
    return ResNet(18, [64, 64, 128, 256], 'basicblock', **kwargs)


def resnet34(**kwargs):
    return ResNet(34, [64, 64, 128, 256], 'basicblock', **kwargs)


def resnet50(**kwargs):
    return ResNet(50, [64, 128, 256, 512], 'bottleneck', **kwargs)


def resnet101(**kwargs):
    return ResNet(101, [64, 128, 256, 512], 'bottleneck', **kwargs)


def resnet152(**kwargs):
    return ResNet(152, [64, 128, 256, 512], 'bottleneck', **kwargs)
