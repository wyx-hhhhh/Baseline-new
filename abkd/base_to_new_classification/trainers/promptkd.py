...
from losses import ABKL, DIST, DistillKL, DKD, TTM
import math
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        # self.text_projection = torch.nn.Parameter(clip_model.text_projection, dtype=torch.float32)
...
class PromptKD(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.distill == 'kd':
            self.criterion = DistillKL(self.temperature)
        elif cfg.distill == 'ab':
            self.criterion = ABKL(cfg)
        elif cfg.distill == 'dist':
            self.criterion = Dist(cfg.dist_alpha, cfg.dist_beta)
        elif cfg.distill == 'ttm':
            self.criterion = TTM(cfg.ttm_l)
        elif cfg.distill == 'dkd':
            self.criterion = DKD(cfg)
        elif cfg.distill == 'ls':
            pass
        else:
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            ...
            if self.cfg.distill == 'kd':
                loss = self.criterion(stu_logits, tea_logits)
                l = self.cfg.TRAINER.PROMPTKD.KD_WEIGHT * loss
            elif self.cfg.distill == 'ab':
                loss = self.criterion(stu_logits, tea_logits)
                l = self.cfg.TRAINER.PROMPTKD.KD_WEIGHT * loss
            elif self.cfg.distill == 'ls':
                def normalize(logit):
                ...
                l = self.cfg.TRAINER.PROMPTKD.KD_WEIGHT * loss
            elif self.cfg.distill == 'dkd':
                l = self.cfg.TRAINER.PROMPTKD.KD_WEIGHT * self.criterion(stu_logits, tea_logits, label, epoch)
            elif self.cfg.distill == 'ttm':
                l = self.cfg.TRAINER.PROMPTKD.KD_WEIGHT * self.criterion(stu_logits, tea_logits)

            optim.zero_grad()
            l.backward()
            optim.step()

        loss_summary = {"loss": l.item()}
