            ...
            def wrapper_mp(*args, **kwargs):
                ...
                def target_func(q, *args, **kwargs):
                    try:
                        ...
                        q.put((False, e))
                # 这里只需要传 q, *args, **kwargs，不要传 func
                process = multiprocessing.Process(target=target_func, args=(q, *args), kwargs=kwargs)
                process.start()
...
def match_answer(response):
    flag = False
    # if "In 3 years, John's youngest son will be \\(37 + 3 = 40\\) years old.\n\nThe final answer is \\(\\boxed{40}\\)." in response:
    #     flag = True
...
def compute_score(model_output, gt_output) -> bool:
    model_output = str(model_output)
    gt_output = str(gt_output)

    is_matched, extracted_model_output = match_answer(model_output)
    _, ground_truth= match_answer(gt_output)
